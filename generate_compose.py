...
Official AgentBeats pattern for local testing and CI/CD.

Usage:
    pip install tomli tomli-w requests
    python generate_compose.py --scenario scenarios/browsergym/scenario.toml
    cp .env.example .env
...
def generate_a2a_scenario(scenario: dict[str, Any]) -> str:
    """Generate a2a-scenario.toml for the agentbeats-client."""
    green = scenario["green_agent"]
    participants = scenario.get("participants", [])
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate Docker Compose from scenario.toml",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    # Generate from default scenario
    python generate_compose.py --scenario scenarios/browsergym/scenario.toml
    ...
    parser.add_argument(
        "--scenario", 
        type=Path,
        default=Path("scenarios/browsergym/scenario.toml"),
    ...
    compose_path = args.output_dir / COMPOSE_PATH
    a2a_path = args.output_dir / A2A_SCENARIO_PATH
    env_path = args.output_dir / ENV_PATH
    ...
    print("\nNext steps:")
    print("  1. Build your agent images:")
    print("     docker build -t weag-green-agent:latest ./green-agent")
    print("     docker build -t weag-purple-agent:latest ./purple-agent")
    print("  2. Copy and edit .env.example:")
    print("     cp .env.example .env")
    print("  3. Run the assessment:")
    print("     mkdir -p output")
    print("     docker compose up --abort-on-container-exit")
