"""A2A conformance tests for WEAG Purple Agent."""
...
# A2A validation helpers (same as green-agent)
...
def validate_event(data: dict[str, Any]) -> list[str]:
    ...
    if kind == 'task':
        ...
        errors = []
        if 'parts' not in data or not isinstance(data.get('parts'), list):
            errors.append("Message object must have a 'parts' array.")
