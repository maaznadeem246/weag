"""
LLM Provider Abstraction Layer for Green Agent.
...
class LLMConfig(BaseModel):
    ...
    @classmethod
    def from_env(cls, prefix: str = "GREEN_") -> "LLMConfig":
        """
        Create configuration from environment variables for Green Agent.
        
        Environment variables (GREEN_ prefixed values win when provided):
        - GREEN_LLM_PROVIDER or LLM_PROVIDER: openai|gemini|litellm (default: gemini)
        - GREEN_OPENAI_API_KEY or OPENAI_API_KEY: OpenAI API key
        - GREEN_GEMINI_API_KEY or GEMINI_API_KEY: Gemini API key
        - GREEN_OPENROUTER_API_KEY or OPENROUTER_API_KEY: OpenRouter API key
        - GREEN_OPENAI_MODEL or OPENAI_MODEL: OpenAI model name
        - GREEN_GEMINI_MODEL or GEMINI_MODEL: Gemini model name
        - GREEN_LITELLM_MODEL or LITELLM_MODEL: LiteLLM model name
        - GREEN_LLM_MAX_ITERATIONS or LLM_MAX_ITERATIONS: Maximum iterations
        """
        ...
        return cls(
            ...
            max_iterations=int(_get("LLM_MAX_ITERATIONS", "50")),
        )
...
class LLMClientFactory:
    ...
    def create_litellm_model(config: LLMConfig) -> Any:
        ...
        if not config.openrouter_api_key:
            raise ValueError(
                ...
                "Please set it in your .env file: GREEN_OPENROUTER_API_KEY or OPENROUTER_API_KEY=your-key-here\n"
    ...
    def _create_gemini_client(config: LLMConfig) -> tuple[AsyncOpenAI, str]:
        ...
        if not config.gemini_api_key:
            raise ValueError(
                ...
                "Please set it in your .env file: GREEN_GEMINI_API_KEY or GEMINI_API_KEY=your-key-here\n"
    ...
    def _create_litellm_client(config: LLMConfig) -> tuple[AsyncOpenAI, str]:
        ...
        logger = logging.getLogger(__name__)
        logger.warning(
            f"⚠ Green Agent LiteLLM: Using fallback AsyncOpenAI client with model='{model_name}'. "
...
def setup_llm_client(config: Optional[LLMConfig] = None, prefix: str = "GREEN_") -> tuple[Optional[AsyncOpenAI], Union[str, Any], LLMConfig]:
    """
    Setup LLM client with configuration for Green Agent.
    ...
    Args:
        config: LLM configuration (if None, loads from environment with GREEN_ prefix)
        prefix: Environment variable prefix (default: "GREEN_")
